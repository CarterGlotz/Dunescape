...
export default function DailyRiteStatus({
    ...
    return (
      <div style={{background:"rgba(40,20,5,0.6)",border:"1px solid #5a3010",borderRadius:4,padding:"6px 4px",textAlign:"center"}}>
        ...
        {contract.latest_outcome&&<div style={{marginTop:5,padding:"5px 6px",background:"rgba(5,12,10,0.58)",border:"1px solid rgba(127,211,166,0.22)",borderRadius:4,textAlign:"left"}}>
          ...
          {contract.latest_outcome.shrine_bargain?.offering_intent&&<div style={{fontSize:7,color:"#ffe6a8",lineHeight:1.35,marginTop:2}}>Offering opened: {contract.latest_outcome.shrine_bargain.offering_intent.target.label} · {contract.latest_outcome.shrine_bargain.offering_intent.next_action}</div>}
  ...
  return (
    <div style={{background:"rgba(40,20,5,0.6)",border:"1px solid rgba(200,168,78,0.2)",borderRadius:4,padding:6}}>
      ...
      {contract.latest_outcome?.shrine_bargain?.offering_intent&&<div style={{fontSize:7,color:"#ffe6a8",lineHeight:1.4,marginBottom:3,textAlign:"center"}}>Offering opened: {contract.latest_outcome.shrine_bargain.offering_intent.target.label} · {contract.latest_outcome.shrine_bargain.offering_intent.summary}</div>}
